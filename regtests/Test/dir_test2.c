...
void 
myPrintEntry(struct bEntryBlock* entry) 
{
...
struct bEntryBlock* 
myGetDirEntry(struct Volume* vol, 
              SECTNUM sector)
{
	size_t entry_size = 
    ...
    memset(dir, 0, entry_size);
           
    RETCODE on_read_dir = 
    adfReadEntryBlock(vol, sector, dir);
...
void 
myWriteDirEntry(struct Volume* vol, 
                SECTNUM sector,
                struct bEntryBlock* dir)  
{
	RETCODE on_write_dir = 
	adfWriteEntryBlock(vol, sector, dir);
...
int main(int argc, char *argv[])
{
    ...
    int dir_hash = 
    adfGetHashValue(dir_name, 0);

    printf("Hash for name [%s] "
           "is [%d].\n", 
           dir_name, 
           dir_hash);
    ...
    SECTNUM mydir_sector = 
    myGetSectNum(adfGetDirEnt(vol, 
                              vol -> 
                              curDirPtr) 
                 -> next 
                 -> content);
